...
def batch_results_tab():
    ...
    if results:
        ...
        st.markdown(results['summary_tables'][0].as_html(), unsafe_allow_html=True) # Model Summary
        st.markdown(results['summary_tables'][1].as_html(), unsafe_allow_html=True) # Coefficients Table
        st.markdown(results['summary_tables'][2].as_html(), unsafe_allow_html=True) # Residual Analysis
