...
def forecast_tab():
    ...
    # --- 顯示公式 ---
    coef_df = current_config['results']['summary_stats'].get('coef_pvalue_df')
    model = perform_ols_regression(st.session_state.df, current_config['y_var'], current_config['x_vars'])
    if coef_df is not None and model is not None:
        formula = [f"{model.params['const']:.4f}"] if 'const' in model.params else []
        for _, row in coef_df.iterrows():
            sign = "+" if row['Coef'] >= 0 else "-"
            formula.append(f"{sign} {abs(row['Coef']):.4f} * {row['Variable']}")
        st.markdown(f"**Y = {' '.join(formula)}**")
